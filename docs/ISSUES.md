# Issues

## What to Report

Please see the [following template](general/bug_report.md) for reporting issues, and note that issues are primarily intended for:

- Bugs or unexpected behaviour
- Reproducible problems with TaskTide
- Documentation errors
- Build or installation problems
- Security issues that should not be reported publicly (see [Reporting Vulnerability](/SECURITY.md))

Before opening an issue, please check existing issues and documentation to avoid duplicates.

<br>

---

<br>

## Feature Requests

Please see the [following template](general/feature_request.md) for feature requests, and note that feature requests are welcome when they are relevant to the scope and goals of TaskTide.

Please describe:

- The problem or use case
- Why the current behaviour is insufficient
- The proposed solution, if you have one
- Any alternatives you have considered

A feature request is a suggestion, not a commitment. The TaskTide project may decline, defer, or close requests that do not fit its scope, design, priorities, or intended use.

<br>

---

<br>

## Scope

TaskTide is developed according to its own design goals and priorities.

An issue does not create an obligation to:

- Implement a requested feature
- Change existing behaviour
- Support a particular use case
- Accept a proposed design
- Maintain compatibility with an external project or workflow
- A resolution SLA for TaskTide

Discussion is welcome, but final decisions about TaskTide remain with the project maintainers.